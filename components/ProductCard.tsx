...
const ProductCard: React.FC<ProductCardProps> = ({ product, addToCart, onClick, language, adminEmails = [] }) => {
  ...
  return (
    <div className="group bg-white dark:bg-slate-800 rounded-3xl overflow-hidden border border-slate-100 dark:border-slate-700 hover:shadow-[0_30px_60px_rgba(0,0,0,0.1)] transition-all duration-500 transform hover:-translate-y-2">
      ...
      <div className="p-6">
        <div className="flex justify-between items-start mb-3">
          <div className="flex flex-col gap-1">
             <div className="flex items-center gap-2">
               <span className="text-[10px] uppercase tracking-[0.2em] font-black text-[#1A237E] dark:text-[#FFD600] bg-blue-50 dark:bg-blue-900/20 px-3 py-1 rounded-md w-fit">
                {product.vendor}
               </span>
               {isAdminEmail && (
                 <span className="bg-[#1A237E] text-white text-[8px] font-black px-2 py-0.5 rounded-full flex items-center gap-1 shadow-sm">
                   <UserCheck className="w-3 h-3" /> OWNER
                 </span>
               )}
               {isVerifiedUser && (
                 <span className="bg-[#FFD600] text-[#1A237E] text-[8px] font-black px-2 py-0.5 rounded-full flex items-center gap-1 shadow-sm">
        ...
        <h3 onClick={onClick} className="font-bold text-slate-800 dark:text-white text-xl mb-3 line-clamp-1 group-hover:text-[#1A237E] dark:group-hover:text-[#FFD600] transition-colors cursor-pointer leading-tight uppercase tracking-tighter">
          {product.name}
        </h3>
        ...
        <button onClick={handleAddToCart} className={`w-full py-4 font-black rounded-2xl border-2 transition-all shadow-lg ${isAdding ? 'bg-green-500 border-green-500 text-white' : 'bg-transparent border-[#1A237E] text-[#1A237E] dark:text-white hover:bg-[#1A237E] hover:text-white'}`}>
          {isAdding ? (language === 'bn' ? 'যোগ হয়েছে' : 'Added') : (language === 'bn' ? 'কার্টে যোগ করুন' : 'Add to Cart')}
        </button>
      </div>
    </div>
  );
